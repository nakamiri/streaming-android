...
```bash
# Widgets ボタンからウィジェットを有効化する手順
# 1. 画面下の Widgets ボタンをタップ
# 2. クイックパネル内の各トグルを切り替える
# 3. 配置変更が必要なら「配置を編集」をタップ
# ※ クイックパネルは配信中でも開ける
# ※ トグルはその場で配信プレビュー/配信映像へ反映される
...
#### カメラ解像度・映像パイプライン
- CameraX の `ResolutionSelector` で配信解像度以上の最も近い 16:9 解像度を自動選択（`FALLBACK_RULE_CLOSEST_HIGHER_THEN_LOWER`）
- カメラ解像度が設定解像度と異なる場合、I420 フレームを設定解像度にスケールしてからエンコード（`YuvUtils.scaleI420`）
- `setTargetRotation` で端末の向きに合わせた回転を適用（横画面では rotation=0）
- エンコーダ初期化時に `inputFormat` から実際のカラーフォーマット・ストライド・スライスハイトを取得し、I420 → NV12 変換 + ストライドパディングを適用（実機 HW エンコーダ対応）
- `Preview` と `ImageAnalysis` に同じ `ResolutionSelector` を適用し、プレビューと配信映像のアスペクト比を一致させる
- ウィジェットオーバーレイは `screenDensity` を使って Compose UI と YUV フレーム上の描画サイズを一致させる
